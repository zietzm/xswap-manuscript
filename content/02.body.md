...
Prediction without reliance on node degree is challenging because many effective methods for edge prediction are correlated with degree (Figure {@fig:node-degree}E).
...
Alternatively, if degree is not a desired feature, then the probability could be used to calibrate features and thus potentially enhance predictive specificity.
